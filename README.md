...
| 4 | Profiling (7 psychological metrics: tactical_vs_positional_bias, time_pressure_quality, opening_comfort, conversion_ability, blunder_rate_vs_rating, decision_fatigue, sequence_based_tilt; corrected from the 2026-08-04 snapshot which listed 6 metrics, profile dashboard, `/profiles/explain`) | Complete | `routes/profile.py`, `routes/profile_analysis.py` |
...
| 6 | PDF / Vision (book ingest via chessvision.ai) | **Partial.** Chessvision.ai integration shipped at `routes/pdf_ingest.py` and `services/chess_coach/gateway/config.py:121`; tests at `tests/unit/test_pdf_ingest_route.py`, `tests/unit/test_pdftomd_metrics.py`, `tests/integration/test_pdf_import.py`. Per `implementation-roadmap-v1.md` § Phase 6 (lines 94-107), the full Phase 6 deliverable list also includes YOLOv8 diagram detector + piece-classifier CNN, PaddleOCR integration, manual-review queue, and user-correction feedback loop; these remain deferred. **Correction (2026-08-13, doc-drift fix-up):** the previous text said "local YOLOv8 + PaddleOCR kept as offline fallback only" but verified against current repo: neither YOLOv8 nor PaddleOCR is present anywhere in services/, libs/, or apps/. The actual PDF OCR path is the external chessvision.ai API only (same false-claim correction as security-strategy.md A-F11 Implementation per FU-19 / PR #95). |
