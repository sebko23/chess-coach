...
5. **OCR primary** — chessvision.ai API is the production path since 2026-06-14 (point-in-time reference; original SHA `6635ffa` is no longer reachable in the current history due to subsequent squash-merges); PaddleOCR vs Tesseract comparison **deferred** (offline fallback only).
...
| 14 specialized agents as separate services | 1 FastAPI monolith (`gateway/`) with 20 route modules under `services/chess_coach/gateway/routes/` (29 `@router.<method>` decorators as of `main@0cf39b6`, 2026-08-04). Module boundaries exist as Python packages but are not network-isolated. `route_guard` cross-cutting decorator (ADR-0002) applied to 25 of 29 route handlers — specifically 6 routes via PR #72 (squash `b320d3b`, reachable); 5 route files remain unguarded (`analysis.py`, `backfill_analyses.py`, `eval_verifier.py`, `pgn_import.py`, `system.py`) — needs its own follow-up BBF. (Earlier `f24bd8b` SHA cited in prior doc versions is no longer reachable; the `route_guard`-applied-to-7-routes claim it supported is superseded by the PR #72 squash count.) |
...
| Qdrant vector database | TF-IDF position-similarity pipeline + in-memory Qdrant-shaped facade landed (point-in-time reference SHA `789b0cd` — no longer reachable in current history due to subsequent squash-merges — see `services/chess_coach/memory_kb/` for the current code location); standalone Qdrant server still not deployed. Embedding-model default (item 2 of §18) carries the open decision on whether to graduate to a real embedding model. |

**What is built and working (as of `main@0cf39b6`, 2026-08-04):** 20 route modules under `services/chess_coach/gateway/routes/` (29 `@router.<method>` decorators), Stockfish 18 engine pool + Maia-1500 via lc0, `engines.json` pre-populated with both (appDataDir copy root cause documented in the historical audit), Sqlite WAL (the 88,452-row figure was a 2026-06-20 snapshot; live row count varies), FSRS spaced repetition, psychological profiling (5 metrics), repertoire gap analysis, **engine-wired narration** (BBF-87.2 squash `e4a4c66` + BBF-87.2.1 squash `0cf39b6` — the narration route now invokes `engine_pool.analyze()` and feeds a real `AnalysisResult` into the LLM prompt; response carries `pv_moves`, `score_display`, `depth_reached`, `best_move` from the engine; real engine failures now propagate to `@route_guard` for a 5xx ADR-0002 envelope, was the silent-fabrication regression BBF-87.2.1 closed), typed OpenAPI TypeScript client, `route_guard` cross-cutting decorator (ADR-0002 — applied to 25 of 29 handlers; 6 routes via PR #72 squash `b320d3b`, plus the rest predating PR #72), chessvision.ai integration for PDF diagram extraction as the production path (Phase 6 partial — chessvision shipped; YOLOv8 + PaddleOCR retained as offline fallback only, per `implementation-roadmap-v1.md` § Phase 6), local pytest on the previously-blocked narration integration suites now passes 9/9 (verified post-FU-1 — the broken `importlib.metadata` for `huggingface-hub` was caused by a ghost `huggingface_hub-1.24.0.dist-info/` orphan that was quarantined during the FU-1 BBF), `activePlayerAtom` shared across Repertoire + TrainingQueue pages, 7 missing storage migrations restored (per `services/chess_coach/storage/migrations/` — check `git log --oneline -- services/chess_coach/storage/migrations/` for the current migration list), project audit at `docs/16_audit/project-audit-2026-06-14.md`, memory-disable durability across 2 session boundaries, and the **57 of 57 integration tests passing** baseline at the `main` HEAD bisect from `75d3af0` through `4feef86` (point-in-time reference — those SHAs are no longer reachable in the current history because subsequent squash-merges have invalidated them; the test count was correct at the time and has since evolved). Per `docs/01_architecture/doc-conventions.md`: SHA citations in current-state claims are intentionally avoided or qualified as point-in-time references; SHAs that are still reachable in `git log --all --reflog` are kept (e.g. `e4a4c66`, `0cf39b6`, `b320d3b`). See also the duplicate copy at `docs/12_claude_review/fable5-prompt1-ready-to-paste.md:1183` — marked there as a historical snapshot; this file is canonical.
