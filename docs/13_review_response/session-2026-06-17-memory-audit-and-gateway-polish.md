...
1. **"Verified across one session boundary" is not "solved forever."** Three durability checks have been performed: one immediate (post-disable), one ~1 hour later (bracketing a 23-entry deletion), and one at the start of a subsequent session (cross-boundary). The cross-boundary check passed, which was the previous qualifier's main gap. What remains untested: a hard container restart (the session boundary may or may not have involved a full process restart — it was at minimum a new chat context, possibly more), and extended observation (weeks to months) to confirm no new fabrications appear over time. The current evidence supports a strong "stable" claim; it does not support a claim of "permanent" or "impossible to regress." Any future session that observes a new John/Max/Agent Zero entry in the pickle should treat it as a regression of the disable, not as a normal state.
