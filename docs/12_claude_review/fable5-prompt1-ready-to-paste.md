...
## Implementation Reality (historical snapshot — FROZEN 2026-06-20)

> **Historical snapshot, NOT canonical.** This block is a frozen copy of `docs/01_architecture/system-architecture.md` § Implementation Reality as it stood on 2026-06-20 (commit `4feef86`). The canonical, current-state version lives at `docs/01_architecture/system-architecture.md`. **Do not edit this file as a way to update the system architecture** — update the canonical doc and re-fan-out the snapshot via the doc-reconciliation BBF pattern (see `docs/01_architecture/doc-conventions.md` "Duplicate doc copies: mark canonical vs snapshot explicitly").

Below is the historical content for review-context purposes only. Several SHAs cited here (`5c05764`, `b7bc0b0`, `cfd6603`, `f24bd8b`, `789b0cd`, `1c171be`, `b1c5bf8`, `6635ffa`, `75d3af0`, `9b590f4`, `d363f7e`, `94b89cd`) are no longer reachable in the current git history due to subsequent squash-merges — per `docs/01_architecture/doc-conventions.md` "SHA citations in current-state docs: avoid or qualify," SHA citations in current-state docs are intentionally avoided or qualified as point-in-time references. The historical content below is preserved verbatim (with a header-clarification banner) so review readers can see what the doc said on 2026-06-20 without confusing it with current state.
